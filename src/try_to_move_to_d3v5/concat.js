...
$( document ).on( "initComplete", function() {
	...
	function switchDomain(domainType) {
		...
		d3.transition().duration(dur*2).on('end', function() {
...
egoGraphVis.prototype.init = function() {
	...
	self.eigenFactorScale = d3.scaleLinear()
	...
	// self.force.force("link", d3.forceLink().id(function(d) { return d.id; }));
	self.force.force("link", d3.forceLink());
...
egoGraphVis.prototype.makeForce = function () {
    var self = this;
    // var _forceSim = d3.forceSimulation()
    ...
    //     // .alpha(0.1)
    //     .on('tick', this.tick);
	var _forceSim = d3.forceSimulation().on('tick', this.tick);
	return _forceSim;
};
...
egoGraphVis.prototype.drawNode = function() {
    ...
    function drawLinks(nodeObj) {
        ...
        nodeObj.linksThisNodeIsSource.classed('hidden', false)
            ...
            // .attr('y2', 0)
            .attr('T', 1)
            .on('end', function(d) { d.inTransition = false; });
    }
...
egoGraphVis.prototype.removeNode = function() {
    ...
    currLinks.transition()
        .on('start', function(d) { d.inTransition=true; })
...
lineChartByYear.prototype.init = function() {
    ...
    self.x = d3.scaleLinear().range([0, self.lineChartDimensions.width]);
    self.y = d3.scaleLinear().range([self.lineChartDimensions.height, 0]);
